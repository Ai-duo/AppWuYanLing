...
public class ElementsService extends Service {
    ...
    public static String sitenum = TAISHUN;

    //天气预报地址
    public static String dayurl = "http://61.153.246.242:8888/qxdata/QxService.svc/getdayybdata/"+sitenum;
    //负氧离子数据地址
    public static String oxurl = "http://115.220.4.68:8081/qxdata/QxService.svc/getqxo2data/k3061";
    Timer weaTimer,oxyTimer;
    private void weaTimer(){
        ...
        oxyTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                OkHttpClient client = new OkHttpClient.Builder().connectTimeout(10, TimeUnit.SECONDS).build();
                String result = null;
                ...
                try {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        JSONObject json = new JSONObject(result);
                       // String Info = json.optString("DATE");
                        String oxy = json.optString("nongdu");
                        // site_name = json.optString("wea_logo");
                        Log.i("TAG","nongdu:"+oxy);
                        if (!TextUtils.isEmpty(oxy)) {
                            EventBus.getDefault().post(new OxygenNd(oxy));
